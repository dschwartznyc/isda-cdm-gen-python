...
class SettlementTerms(SettlementBase):
  ...
  def condition_0_OptionSettlementChoice(self):
    ...
    """
    def _then_fn0():
      return (all_elements(self.settlementType, "=", SettlementTypeEnum.ELECTION) or all_elements(self.settlementType, "=", SettlementTypeEnum.CASH_OR_PHYSICAL))
    
    def _else_fn0():
      return True
    
    return if_cond_fn((((self.cashSettlementTerms) is not None) and ((self.physicalSettlementTerms) is not None)), _then_fn0, _else_fn0)
